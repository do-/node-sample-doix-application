...
doUpdate:
    ...
    async function () {
        ...
        let toSpawn = conf.workers
            
        for (const worker of Object.values (cluster.workers)) if (!worker.isDead ()) toSpawn --

        const toDo = []; for (let i = 0; i < toSpawn; i ++) toDo.push (newWorker ({}))

        await Promise.all (toDo)

        for (const worker of Object.values (cluster.workers)) if (!worker.isDead ()) worker.send ({type: 'worker_process', action: 'update'})

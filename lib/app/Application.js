...
const HeartBeat    = require ('./MessageSources/Q/HeartBeat')
const HttpDia      = require ('./MessageSources/HTTP/Dia')
const HttpRest     = require ('./MessageSources/HTTP/Rest')
const HttpFallback = require ('./MessageSources/HTTP/Fallback')

const DbDw         = require ('./DbPools/DbDw')
...
module.exports = class extends Application {
	...
	initHttpRouter () {
		...
		const {logger, globals} = this, {http: {listen}} = globals.get ('conf')

		const o = {listen, logger, name: 'http_' + process.pid}

		this.httpRouter = new HttpRouter (o)

		this.httpRouter.add (new HttpDia      (this))
		this.httpRouter.add (new HttpRest     (this))
		this.httpRouter.add (new HttpFallback (this))
